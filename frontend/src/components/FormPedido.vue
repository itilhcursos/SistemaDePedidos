...
export default {
    ...
    methods: {
        async salvarpedido() {
            ...
            if (this.id === "") {
                ...
                const response = await axios.put(`http://localhost:8080/pedido/${this.id}`,
                    {
                        id: this.id,
                        descricao: this.descricao,
                        quantidadeEstoque: this.quantidadeEstoque,
    ...
    mounted() {
        if (this.propspedido) {
            ...
            this.descricao = this.propspedido.descricao;
            this.quantidadeEstoque = this.propspedido.quantidadeEstoque;
            this.precoUnidadeAtual = this.propspedido.precoUnidadeAtual;
            this.ativo = this.propspedido.ativo;
        }
    },
