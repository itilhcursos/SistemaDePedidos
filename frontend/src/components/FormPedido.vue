          ...
          <table class="table table-dark table-striped">
                ...
                <tbody>
                  <tr v-for="item in itens" :key="item.id" scope="row">
                    ...
                    <th>
                      {{ formatarReaisTela(item.quantidadeEstoque * item.precoUnidadeAtual) }}
                    </th>
                    <th>
                      <button
          ...
          <div class="col-8">
            ...
            <v-select class="meu-select" v-model="selectedProduto" :filterable="false" :options="optionsProduto"
              ...
              <template v-slot:option="option">
                ...
                {{ option.descricao }} <pre> Estoque ({{ option.quantidadeEstoque }}), Preço ({{ formatarReaisTela(option.precoUnidadeAtual) }})</pre>
              </template>
              <template v-slot:selected-option="option">
                <img class="mini" :src='option.urlImagem' />
                {{ option.descricao }} &emsp;|&emsp;Estoque: {{ option.quantidadeEstoque }}, &nbsp; Preço: {{ formatarReaisTela(option.precoUnidadeAtual) }}
              </template>
            </v-select>
          </div>
          <div class="col-2">
